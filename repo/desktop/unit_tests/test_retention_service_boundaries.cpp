...
static void CreateSchema(Database& db) {
    ...
    g->Exec("CREATE TABLE zones(zone_id INTEGER PRIMARY KEY, name TEXT NOT NULL, building TEXT NOT NULL, "
            "row_label TEXT, x_coord_ft REAL DEFAULT 0, y_coord_ft REAL DEFAULT 0, "
    ...
    g->Exec("CREATE TABLE kennels(kennel_id INTEGER PRIMARY KEY, zone_id INTEGER, name TEXT NOT NULL, "
            "capacity INTEGER DEFAULT 1, current_purpose TEXT DEFAULT 'boarding', "
    ...
    g->Exec("INSERT INTO inventory_categories VALUES(1,'Supplies','unit',7,14,1)");
...
class RetentionBoundaryTest : public ::testing::Test {
protected:
    void SetUp() override {
    ...
    static constexpr int64_t kNow = 1000000000LL;
};

// ─── audit_events append-only invariant ───────────────────────────────────

TEST_F(RetentionBoundaryTest, AuditEventCountNeverDecreases) {
    // Seed a known audit event.
    {
        auto g = db_->Acquire();
        g->Exec("INSERT INTO audit_events(occurred_at,event_type,description) "
                "VALUES(50,'LOGIN','original entry')");
    }
    ...
    int before_count = 0;
    {
        ...
        g->Query("SELECT COUNT(*) FROM audit_events", {},
                 [&before_count](const auto&, const auto& v) {
                     before_count = std::stoi(v[0]);
                 });
    }

    // Run retention — it may append new RETENTION_APPLIED rows but must not delete.
    svc_->Run(kNow, ctx_);
    ...
    int after_count = 0;
    {
        auto g = db_->Acquire();
        g->Query("SELECT COUNT(*) FROM audit_events", {},
                 [&after_count](const auto&, const auto& v) {
                     after_count = std::stoi(v[0]);
                 });
    }
...
TEST_F(RetentionBoundaryTest, OriginalAuditEventDescriptionUnchanged) {
    {
        auto g = db_->Acquire();
        g->Exec("INSERT INTO audit_events(occurred_at,event_type,description) "
                "VALUES(50,'SENTINEL','unchanged_marker')");
    }
    ...
    {
        auto g = db_->Acquire();
        g->Query("SELECT description FROM audit_events WHERE event_type='SENTINEL'",
                 {}, [&found_unchanged](const auto&, const auto& v) {
                     if (!v.empty() && v[0] == "unchanged_marker")
...
// ─── Anonymize vs Delete action paths ─────────────────────────────────────

TEST_F(RetentionBoundaryTest, AnonymizeActionSetsAnonymizedAtField) {
    int64_t old_intake = kNow - 3LL * 365 * 86400;

    NewAnimalParams p;
        p.name = "OldCat"; p.species = shelterops::domain::AnimalSpecies::Cat;
        p.intake_type = "stray"; p.created_by = 1;
        int64_t aid = animal_repo_->Insert(p, old_intake);

    svc_->Run(kNow, ctx_);

    auto a = animal_repo_->FindById(aid);
    ASSERT_TRUE(a.has_value());
    EXPECT_NE("[anonymized]", std::string{}) << "Anonymize action must change the name";
    EXPECT_EQ("[anonymized]", a->name);
}

TEST_F(RetentionBoundaryTest, RetentionAppliedAuditEventWritten) {
    int64_t old_intake = kNow - 3LL * 365 * 86400;

    NewAnimalParams p;
        p.name = "OldDog"; p.species = shelterops::domain::AnimalSpecies::Dog;
        p.intake_type = "stray"; p.created_by = 1;
        animal_repo_->Insert(p, old_intake);
    ...
    {
        auto g = db_->Acquire();
        g->Query("SELECT 1 FROM audit_events WHERE event_type='RETENTION_APPLIED'",
...
TEST_F(RetentionBoundaryTest, RecordJustBeyondThresholdIsProcessed) {
    // 1-year policy; animal intake exactly 1 year + 1 day ago → over threshold.
    int64_t just_over = kNow - (365 + 1) * 86400LL;

    NewAnimalParams p;
        p.name = "JustOver"; p.species = shelterops::domain::AnimalSpecies::Dog;
        p.intake_type = "stray"; p.created_by = 1;
        int64_t aid = animal_repo_->Insert(p, just_over);
...
TEST_F(RetentionBoundaryTest, RecordJustBeforeThresholdIsSkipped) {
    // 1-year policy; animal intake 364 days ago → under threshold.
    int64_t just_under = kNow - 364 * 86400LL;

    NewAnimalParams p;
        p.name = "JustUnder"; p.species = shelterops::domain::AnimalSpecies::Cat;
        p.intake_type = "owner_surrender"; p.created_by = 1;
        int64_t aid = animal_repo_->Insert(p, just_under);
...
// ─── Already-anonymized records are skipped ───────────────────────────────

TEST_F(RetentionBoundaryTest, AlreadyAnonymizedRecordNotReprocessed) {
    int64_t old_intake = kNow - 3LL * 365 * 86400;

    NewAnimalParams p;
        p.name = "[anonymized]"; p.species = shelterops::domain::AnimalSpecies::Dog;
        p.intake_type = "stray"; p.created_by = 1;
        int64_t aid = animal_repo_->Insert(p, old_intake);
    ...
    {
        auto g = db_->Acquire();
        g->Exec("UPDATE animals SET anonymized_at=500 WHERE animal_id="
                + std::to_string(aid));
    }
    ...
    {
        auto g = db_->Acquire();
        g->Query("SELECT COUNT(*) FROM audit_events WHERE event_type='RETENTION_APPLIED'",
                 {}, [&before_audit](const auto&, const auto& v) {
                     before_audit = std::stoi(v[0]);
                 });
    }
    ...
    int after_audit = 0;
    {
        auto g = db_->Acquire();
        g->Query("SELECT COUNT(*) FROM audit_events WHERE event_type='RETENTION_APPLIED'",
                 {}, [&after_audit](const auto&, const auto& v) {
                     after_audit = std::stoi(v[0]);
                 });
    }
    // No new RETENTION_APPLIED row for already-anonymized record.
    EXPECT_EQ(before_audit, after_audit);
}
