...
// Surface: CommandDispatcher, the internal contract for the future HTTP adapter.
//
// Purpose: Enumerate the authoritative set of registered commands and verify
// the authorization surface at the role boundary.  Every row in the table
// below maps a command name to:
//   - the minimum role that may invoke it successfully
//   - a role that must be rejected with 403 FORBIDDEN
//
// NOTE: This test does not start a real HTTP server. It calls the dispatcher
// directly, identical to the in-process contract tests in unit_tests/.
// It lives in api_tests/ because it represents the stable automation surface
// inventory that a future HTTP adapter must respect.
// =============================================================================
...
static void CreateFullSchema(Database& db) {
    ...
    g->Exec("INSERT INTO zones VALUES(1,'Z','B',NULL,0,0,NULL,1)");
    ...
    g->Exec("INSERT INTO kennels(kennel_id,zone_id,name,capacity,current_purpose,"
            "nightly_price_cents,rating,is_active,notes) VALUES(1,1,'K1',1,'boarding',5000,0,1,null)");
    ...
    g->Exec("INSERT INTO inventory_categories(category_id,name,unit,low_stock_threshold_days,"
            "expiration_alert_days,is_active) VALUES(1,'Supplies','unit',7,14,1)");
    ...
    g->Exec("INSERT INTO inventory_items(item_id,category_id,name,description,storage_location,"
            "quantity,unit_cost_cents,expiration_date,serial_number,barcode,is_active,"
            "created_at,updated_at,anonymized_at) VALUES(1,1,'Test Item',null,null,10,0,null,null,null,1,1,1,null)");
...
class CommandSurfaceInventoryTest : public ::testing::Test {
protected:
    void SetUp() override {
                std::error_code ec;
                std::filesystem::remove("test_command_surface_inventory.db", ec);
                db_           = std::make_unique<Database>("test_command_surface_inventory.db");
    ...
    CommandEnvelope MakeEnv(const std::string& tok, const std::string& cmd,
                             nlohmann::json body = {}) {
        CommandEnvelope e;
        e.session_token    = tok;
        e.command          = cmd;
...
// Every command that the in-process dispatcher handles must appear in this
// table. A static reviewer can audit the surface by reading this test.
// =============================================================================

struct CommandEntry {
    std::string name;                  // command identifier
    std::string permitted_session;     // session token for a role allowed to invoke it
    std::string forbidden_session;     // session token for a role that must be blocked
    nlohmann::json permitted_body;     // minimal valid body for the permitted role
};

// Table: command → (permitted role, forbidden role, minimal body)
// "no-forbidden" = all roles may invoke (read-only commands).
static const std::vector<CommandEntry> kCommandTable = {
    // Search is open to all roles (returns kennel list).
    { "kennel.search",     "tok-auditor",  "",            {{"check_in_at",1000},{"check_out_at",2000}} },
    // Booking create requires non-auditor write role.
    { "booking.create",    "tok-manager",  "tok-auditor", {{"kennel_id",1},{"check_in_at",1000},{"check_out_at",2000}} },
    // Booking approve requires operations_manager or administrator.
    // booking.create runs first in this table and creates booking_id=1.
    { "booking.approve",   "tok-manager",  "tok-clerk",   {{"booking_id",1}} },
    // Booking cancel requires any write role.
    { "booking.cancel",    "tok-manager",  "tok-auditor", {{"booking_id",1}} },
    // Inventory issue requires inventory_clerk or higher.
    { "inventory.issue",   "tok-clerk",    "tok-auditor", {{"item_id",1},{"quantity",1}} },
    // Inventory receive requires inventory_clerk or higher.
    { "inventory.receive", "tok-clerk",    "tok-auditor", {{"item_id",1},{"quantity",1}} },
    // Report trigger requires Operations Manager or Administrator.
    { "report.trigger",    "tok-admin",    "tok-clerk",   {{"report_id",1}} },
    // Report status is open to all authenticated users.
    { "report.status",     "tok-auditor",  "",            {{"run_id",1}} },
    // Export request — tested via dedicated ExportService tests; no forbidden here.
    { "export.request",    "tok-admin",    "",            {{"run_id",1},{"format","csv"}} },
    // Alerts list is read-only, open to all.
    { "alerts.list",       "tok-auditor",  "",            {} },
    // Alerts dismiss requires Operations Manager or Administrator.
    { "alerts.dismiss",    "tok-manager",  "tok-auditor", {{"alert_id",9999}} },
    // Update import requires Administrator only; no UpdateManager injected → returns 500 not 404.
    { "update.import",     "tok-admin",    "tok-manager", {{"msi_path","/fake/pkg.msi"}} },
};

TEST_F(CommandSurfaceInventoryTest, AllCommandsAreRegistered) {
    // Every command in the table must return non-404 for a valid session.
    for (const auto& entry : kCommandTable) {
        auto env = MakeEnv(entry.permitted_session, entry.name, entry.permitted_body);
        auto result = dispatcher_->Dispatch(env, 5000);
        EXPECT_NE(404, result.http_status)
            << "Command '" << entry.name << "' returned 404 — not registered";
    }
}

TEST_F(CommandSurfaceInventoryTest, AllCommandsReturnJsonObject) {
    for (const auto& entry : kCommandTable) {
        auto env = MakeEnv(entry.permitted_session, entry.name, entry.permitted_body);
        auto result = dispatcher_->Dispatch(env, 5000);
        EXPECT_TRUE(result.body.is_object())
            << "Command '" << entry.name << "' did not return a JSON object";
    }
}

TEST_F(CommandSurfaceInventoryTest, ForbiddenRolesReceive403) {
    for (const auto& entry : kCommandTable) {
        if (entry.forbidden_session.empty()) continue; // no forbidden role for this command
        auto env = MakeEnv(entry.forbidden_session, entry.name, entry.permitted_body);
        auto result = dispatcher_->Dispatch(env, 5000);
        EXPECT_EQ(403, result.http_status)
            << "Command '" << entry.name << "' should return 403 for forbidden session '"
            << entry.forbidden_session << "' but returned " << result.http_status;
    }
}

TEST_F(CommandSurfaceInventoryTest, UnknownCommandReturns404) {
    auto result = dispatcher_->Dispatch(MakeEnv("tok-admin", "nonexistent.cmd"), 5000);
    EXPECT_EQ(404, result.http_status);
}

TEST_F(CommandSurfaceInventoryTest, InvalidSessionReturns401ForAllCommands) {
    for (const auto& entry : kCommandTable) {
        auto env = MakeEnv("invalid-tok", entry.name, entry.permitted_body);
        auto result = dispatcher_->Dispatch(env, 5000);
        EXPECT_EQ(401, result.http_status)
            << "Command '" << entry.name << "' did not return 401 for invalid session";
    }
}

TEST_F(CommandSurfaceInventoryTest, CommandInventoryHasTwelveEntries) {
    // Regression guard: any addition or removal must update this table.
    EXPECT_EQ(12u, kCommandTable.size());
}
