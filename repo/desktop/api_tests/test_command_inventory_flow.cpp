...
static void SetupInventoryDb(Database& db) {
    auto g = db.Acquire();
    ...
    g->Exec("INSERT INTO users VALUES(1,'clerk','Clerk','h','inventory_clerk',1,1)");
    ...
    g->Exec("CREATE TABLE user_sessions(session_id TEXT PRIMARY KEY, "
        ...
        g->Exec("INSERT INTO user_sessions VALUES('tok-clerk',1,1,9999999999,'fp',1,9999999999)");
        g->Exec("INSERT INTO user_sessions VALUES('tok-auditor',2,1,9999999999,'fp',1,9999999999)");
    ...
    g->Exec("INSERT INTO inventory_items(item_id,category_id,name,quantity,created_at,updated_at)"
            " VALUES(1,1,'Dog Food',50,1,1)");
...
class InventoryFlowApiTest : public ::testing::Test {
protected:
    void SetUp() override {
        db_ = std::make_unique<Database>(":memory:");
        SetupInventoryDb(*db_);
...
TEST_F(InventoryFlowApiTest, ReceiveStockReturns200) {
    CommandEnvelope env;
    env.command       = "inventory.receive";
    env.session_token = "tok-clerk";
...
TEST_F(InventoryFlowApiTest, IssueStockReturns200) {
    CommandEnvelope env;
    env.command       = "inventory.issue";
    env.session_token = "tok-clerk";
        env.device_fingerprint = "fp";
    env.body          = {
        {"item_id",  1},
        {"quantity", 5},
        {"reason",   "morning feeding"}
    };
...
TEST_F(InventoryFlowApiTest, AuditorInventoryIssueReturns403) {
    CommandEnvelope env;
    env.command       = "inventory.issue";
    env.session_token = "tok-auditor";
        env.device_fingerprint = "fp";
    env.body          = {{"item_id", 1}, {"quantity", 1}, {"reason", "test"}};
...
TEST_F(InventoryFlowApiTest, InventoryIssueMissingFieldsReturns400) {
    CommandEnvelope env;
    env.command       = "inventory.issue";
    env.session_token = "tok-clerk";
        env.device_fingerprint = "fp";
    env.body          = {{"reason", "test"}};  // missing item_id and quantity
