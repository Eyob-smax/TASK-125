...
static void SetupThrottleDb(Database& db) {
    auto g = db.Acquire();
...
// Uses a 1-RPM tight limiter to verify rate-limit enforcement.
class ExportThrottlingApiTest : public ::testing::Test {
protected:
    void SetUp() override {
        ...
        auto booking_svc  = std::make_unique<BookingService>(
        ...
        // 1 RPM — first call passes, second is immediately throttled.
        tight_limiter_    = std::make_unique<RateLimiter>(1);

        tight_dispatcher_ = std::make_unique<CommandDispatcher>(
            *booking_svc, *inv_svc, *report_svc, *export_svc, *alert_svc,
            *session_repo_, *user_repo, *tight_limiter_, *audit_svc);
        ...
        booking_svc_  = std::move(booking_svc);
    ...
    std::unique_ptr<RateLimiter>           tight_limiter_;
    std::unique_ptr<CommandDispatcher>     tight_dispatcher_;
};

TEST_F(ExportThrottlingApiTest, SecondRapidRequestReturns429) {
    CommandEnvelope env;
    env.command       = "alerts.list";
    env.session_token = "tok-admin";
        env.device_fingerprint = "fp";
    env.body          = {};

    // First request passes.
    auto first = tight_dispatcher_->Dispatch(env, 5000);
    EXPECT_EQ(200, first.http_status);

    // Second immediate request to the same session token is throttled.
    auto second = tight_dispatcher_->Dispatch(env, 5000);
    EXPECT_EQ(429, second.http_status);
    EXPECT_FALSE(second.body.value("ok", true));
}

TEST_F(ExportThrottlingApiTest, ExportRequestMissingRunIdReturns400) {
    // Use a normal 60-RPM dispatcher for the 400 test.
    auto normal_limiter = std::make_unique<RateLimiter>(60);
    ...
    auto audit_repo   = std::make_unique<AuditRepository>(*db_);
    auto audit_svc    = std::make_unique<AuditService>(*audit_repo);
    auto local_vault  = std::make_unique<InMemoryCredentialVault>();
    auto booking_svc  = std::make_unique<BookingService>(*kennel_repo, *booking_repo, *admin_repo, *local_vault, *audit_svc);
    auto inv_svc      = std::make_unique<InventoryService>(*inv_repo, *audit_svc);
    auto report_svc   = std::make_unique<ReportService>(*report_repo, *kennel_repo, *booking_repo, *inv_repo, *maint_repo, *audit_svc);
    auto export_svc   = std::make_unique<ExportService>(*report_repo, *admin_repo, *audit_svc);
    auto alert_svc    = std::make_unique<AlertService>(*inv_repo, *audit_svc);

    CommandDispatcher normal_dispatcher(
        *booking_svc, *inv_svc, *report_svc, *export_svc, *alert_svc,
        *session_repo_, *user_repo, *normal_limiter, *audit_svc);

    CommandEnvelope env;
    env.command       = "export.request";
    env.session_token = "tok-admin";
        env.device_fingerprint = "fp";
    env.body          = {{"format", "csv"}};  // missing run_id

    auto result = normal_dispatcher.Dispatch(env, 5000);
