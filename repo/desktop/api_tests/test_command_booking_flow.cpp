...
static void SetupBookingDb(Database& db) {
    auto g = db.Acquire();
...
class BookingFlowApiTest : public ::testing::Test {
protected:
    void SetUp() override {
...
TEST_F(BookingFlowApiTest, BookingCreateReturns201WithBookingId) {
    CommandEnvelope env;
    env.command       = "booking.create";
    ...
    EXPECT_EQ(201, result.http_status);
    EXPECT_TRUE(result.body.value("ok", false));
    ASSERT_TRUE(result.body.contains("data"));
    EXPECT_GT(result.body["data"].value("booking_id", int64_t{0}), 0);
}

TEST_F(BookingFlowApiTest, BookingCancelAfterCreate) {
    // Create
    CommandEnvelope create_env;
    create_env.command       = "booking.create";
    create_env.session_token = "tok-admin";
        create_env.device_fingerprint = "fp";
    create_env.body          = {
        {"kennel_id",    1},
        {"check_in_at",  30000},
        {"check_out_at", 40000},
    ...
    auto create_res = dispatcher_->Dispatch(create_env, 5000);
    ASSERT_EQ(201, create_res.http_status);
    int64_t booking_id = create_res.body["data"].value("booking_id", int64_t{0});
    ASSERT_GT(booking_id, 0);

    // Cancel
    CommandEnvelope cancel_env;
    cancel_env.command       = "booking.cancel";
    cancel_env.session_token = "tok-admin";
        cancel_env.device_fingerprint = "fp";
    cancel_env.body          = {{"booking_id", booking_id}};

    auto cancel_res = dispatcher_->Dispatch(cancel_env, 5000);
    EXPECT_EQ(200, cancel_res.http_status);
    EXPECT_TRUE(cancel_res.body.value("ok", false));
}

TEST_F(BookingFlowApiTest, BookingCreateMissingKennelIdReturns400) {
    CommandEnvelope env;
    env.command       = "booking.create";
    env.session_token = "tok-admin";
        env.device_fingerprint = "fp";
    env.body          = {{"check_in_at", 10000}, {"check_out_at", 20000}};
