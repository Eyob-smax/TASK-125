...
static void SetupAuthzDb(Database& db) {
    auto g = db.Acquire();
...
class CommandAuthorizationApiTest : public ::testing::Test {
protected:
    void SetUp() override {
...
TEST_F(CommandAuthorizationApiTest, InvalidSessionReturns401) {
    CommandEnvelope env;
    env.command       = "kennel.search";
    env.session_token = "bad-token-does-not-exist";
...
TEST_F(CommandAuthorizationApiTest, AuditorBookingCreateReturns403) {
    CommandEnvelope env;
...
TEST_F(CommandAuthorizationApiTest, AuditorInventoryReceiveReturns403) {
    CommandEnvelope env;
    env.command       = "inventory.receive";
    env.session_token = "tok-auditor";
        env.device_fingerprint = "fp";
    env.body          = {{"item_id", 1}, {"quantity", 10}};
...
TEST_F(CommandAuthorizationApiTest, AdminBookingCreateSucceeds) {
    CommandEnvelope env;
    env.command       = "booking.create";
